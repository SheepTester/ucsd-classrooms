import { Quarter, termCode } from "../../terms/index.js";
import {
  coursesFromFile,
  CourseFormatError,
  TermCourses,
} from "./coursesFromFile.js";
...
export class TermCache {
  ...
  #request(
    ...
    const cached =
      this.#cache[termId] ?? this.#cache[this.#term(request, true)];
  ...
  requestTerms(
    ...
    if (allCached) {
      ...
      return Promise.all(rawResults).then((results) =>
        this.#partition(results)
      );
    }
  }
}
